...
async def add_admin(pool, admin_id):
    async with pool.acquire() as connection:
        await connection.execute("INSERT INTO admins (user_id) VALUES ($1) ON CONFLICT DO NOTHING", admin_id)

async def is_admin(pool, user_id):
    async with pool.acquire() as connection:
        admin_id = await connection.fetchval("SELECT user_id FROM admins WHERE user_id = $1", user_id)
